...
namespace GameCreator.Runtime.Common
{
    ...
    [Example(
        "A linear interpolation is a value proportional to any intermediate points between " +
    ...
    public class GetPositionMathLinearInterpolate : PropertyTypeGetPosition
    {
        ...
        public override Vector3 Get(Args args)
        {
            ...
            {
                ClampMode.Clamp => Vector3.Lerp(position1, position2, interpolation),
                ClampMode.Overshoot => Vector3.LerpUnclamped(position1, position2, interpolation),
                _ => throw new ArgumentOutOfRangeException()
            };
        }

        public static PropertyGetPosition Create() => new PropertyGetPosition(
            new GetPositionMathLinearInterpolate()
        );
